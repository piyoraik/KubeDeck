...
**分母が何なのかを書く。** 数字だけ並べると requests と allocatable のどちらを見ているのか分からない。詳細パネルは `1m / 要求 100m`、`120m / 割り当て可能 8` と書く。一覧の列名も `CPU / 要求` にする。

**limits を 2 本目の棒にしない。** 上限は要求より大きいのがふつうで、要求を基準にした棒には収まらない。割合を 2 つ並べても、どちらの分母を見ているのか分からなくなるだけ。棒の下に数字で 1 行足す（`上限 340Mi`）。

**未設定を空欄にしない。** requests が無ければスケジューラは置き場所を決める根拠を持たず、limits が無ければノードの空きまで使える。どちらも「設定されているが 0」とは意味が違うので、`上限 未設定（ノードの空きまで使えます）` と書いて薄く出す。使用量が上限を超えたら、その行を状態色にする。
