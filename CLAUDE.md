...
- **`--max-log-requests` を既定のままにしない。** 既定は **5**（同上）で、
  ...
  黙らない** —— 掴んでいる数は先に分かっているので、帯に
  「N 個中 30 個までしか追いかけられません」と書く（「無い」と「取れていない」を
  混ぜない、と同じ話。出ていない Pod は「何も出していない」に見える）。
...
`[2026-08-02 01:23:45]` も通らない。**書式（段の数）は未確認**なので、
段を数えず囲みの中身をそのまま出どころにしてある（`LogLineTests`）。

**Pod ごとに色を塗らない。** 読みやすそうに見えるが、行の帯は深刻度（状態の
4 色）が持っている場所なので、隣で別の意味の色が動くと、どちらが状態なのか
分からなくなる。出どころは灰色の列と、指したときのツールチップ。
...
**コピーには出どころを付ける**（`[pod/container] 本文`）。混ぜて読んでいるので、
貼った先で出どころが落ちるとまとめ読みの結果として使えない。
...
**未確認**（実測してから直すこと）—— prefix の実際の書式（`[<pod>/<container>] `
だと読んでいるが突き合わせていない）、`--follow` 中に新しく生まれた Pod を拾うか
（ロールアウト中）、`--max-log-requests` 超過時の文言。確かめるなら
`kubectl create deployment demo --image=registry.k8s.io/pause:3.9 --replicas=3` で
足りる。**確かめたら消すこと。**
