...
行ごとにずれる。太さも外観設定で変わらない。細い矩形で引く
（`BranchConnector`）。ただし**座標は計算しない** — 引くのは固定幅の桁の中の
縦線と横線だけで、折り返しやスクロールに追従させる仕組みは持たない。
...
**見方（ノード別 / ワークロード別）は設定に置かない。** 画面の上の切り替えで選ぶ。
「どこに載っているか」と「どこに散っているか」は同じ画面で行き来しながら見るもので、
そのたびに設定を開くのでは使えない。設定に残すのは、いちど決めたら変えない類の
もの（タイルの大きさ、棒が表すもの、空のノードの扱い）だけ。**値の持ち場は
`Preferences` のままで、置き場所だけが違う。**
...
**座標を計算して線を引かない。** 枝は罫線の文字（`├─` `└─` `│`）で描く。
自前で線を引くと、折り返しやスクロールのたびに位置を計算し直すことになり、
図のためだけにレイアウトの仕組みを持つことになる。
